...
| C1-S2 | Politici `ai_jobs`/`credite_ai` + înregistrare RLS | ⚠️ PARȚIAL | `8faa464b` | Înregistrarea `ai_jobs`+`credite_ai` în `TABELE_CU_RLS_UTILIZATOR` e PREPARATORIAL: pregătește calea ca `tabelUtilizator()` să le poată servi prin clientul cu RLS. Politici `auth.uid()=user_id` confirmate pe toate tabelele; `barcode_cache`/`clerk_user_map` corect admin-only. Funcțional `înregistrarea` în sine NU blochează by-pass-ul: un `supabaseAdmin.from(...)` direct ocolește în continuare RLS. Eliminarea reală vine din C1-S3 (regula ESLint) + C1-S1 (calea RLS pentru Clerk). | Nu este autosuficient: dependență de C1-S3 pentru interdicție și de C1-S4 pentru vizibilitate. |
