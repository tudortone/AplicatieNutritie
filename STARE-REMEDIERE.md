...
| ID | Verdict | Status | Commit | Cum s-a rezolvat | Ce a rămas |
| --- | --- | --- | --- | --- | --- |
| P-15 | Test RLS neconectat în CI (auto-skip) | 🔴 activ | ✅ | `fix(P-15)` | Aplicat migrările SQL din `supabase/migrations/*.sql` pe containerul Postgres din CI prin `psql` cu `ON_ERROR_STOP=1` | — |
| P-16/17 | Fără audit/dep-review/secret-scan/EAS + fallback-uri dummy | 🔴 activ | ⚠️ PARȚIAL | `fix(P-16)` | Adaptat truffleHog `base: ${{ github.event.before }}` și rulate testele frontend cu `npm run test:ci` | Job EAS build automatizat rămas de integrat |
