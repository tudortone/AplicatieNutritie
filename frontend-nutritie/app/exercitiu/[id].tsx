...
export default function ExercitiuDetailScreen() {
  ...
  // FIT-003: seturile de încălzire nu intră în volumul de lucru / PR.
  ...
  const sessionLoad = computeSessionLoad(spec, {
    // Dacă există seturi logate dar niciunul de lucru, volumul e 0 (nu preview).
    sets: workingSets.length > 0 ? workingSets.length : (hasLoggedSets ? 0 : 3),
    reps: workingSets.length > 0 ? Math.max(...workingSets.map(s => s.repetari)) : (hasLoggedSets ? 0 : 10),
    weightKg: workingSets.length > 0 ? Math.max(...workingSets.map(s => s.greutate || 0)) : 0,
    // FIT-003: nu există timp real de sesiune în acest ecran (doar estimări),
    // deci păstrăm durata implicită per set din specificație; 60 e fallback.
    durationSec: spec.defaultDurationSec ?? 60,
    bodyweightKg: greutateKg || 75,
  });
  const volumTotal = Math.round(sessionLoad);

  const maxWeight = workingSets.length > 0 ? Math.max(...workingSets.map(s => s.greutate || 0)) : 0;
  const totalReps = workingSets.reduce((sum, s) => sum + s.repetari, 0);
  const scorIntensitate = Math.min(100, Math.max(15, Math.round(
    (maxWeight * 0.95) + (totalReps * 0.8) + (workingSets.length * 6.5) + (exercitiu.dificultate === 'greu' ? 18 : exercitiu.dificultate === 'mediu' ? 10 : 0)
  )));
