    ...
    <View style={[styles.container, { backgroundColor: colors.background, paddingTop: insets.top }]}>
      <View style={[styles.header, { borderBottomColor: colors.border }]}>
        <Pressable
          ...
          style={[styles.iconButton, { backgroundColor: colors.surfaceBg, borderColor: colors.border }]}
        >
        ...
        <Pressable
          onPress={() => router.push('/jurnal-antrenamente')}
          accessibilityRole="button"
          accessibilityLabel="Deschide jurnalul complet de antrenamente"
          style={[styles.iconButton, { backgroundColor: colors.surfaceBg, borderColor: colors.border }]}
        >
      ...
      {loading ? (
        <View style={styles.loading}>
          ...
          <Text style={{ color: colors.textSecondary }}>Se încarcă progresul…</Text>
        </View>
      ) : (
        <FlatList
          data={antrenamente}
          ...
          ListHeaderComponent={
            <View style={styles.topContent}>
              ...
              <View style={styles.summaryRow}>
                <View style={[styles.summary, { backgroundColor: colors.surfaceBg, borderColor: colors.border }]}>
                  <Text style={[styles.summaryValue, { color: colors.accent }]}>{snapshot.exercises}</Text>
                  <Text style={[styles.summaryLabel, { color: colors.textSecondary }]}>exerciții</Text>
                </View>
                <View style={[styles.summary, { backgroundColor: colors.surfaceBg, borderColor: colors.border }]}>
                  <Text style={[styles.summaryValue, { color: colors.accentSecondary }]}>{snapshot.sets}</Text>
                  <Text style={[styles.summaryLabel, { color: colors.textSecondary }]}>seturi</Text>
                </View>
                <View style={[styles.summary, { backgroundColor: colors.surfaceBg, borderColor: colors.border }]}>
                  <Text style={[styles.summaryValue, { color: colors.warning }]}>{Math.round(snapshot.volumeKg)}</Text>
                  <Text style={[styles.summaryLabel, { color: colors.textSecondary }]}>kg volum</Text>
                </View>
              </View>

              <View style={styles.sectionHeader}>
                <Dumbbell size={18} color={colors.accent} />
                <Text style={[styles.sectionTitle, { color: colors.textPrimary }]}>Antrenamentele de azi</Text>
              </View>
            </View>
          }
          ListEmptyComponent={
          ...
          renderItem={({ item }) => {
            ...
            const exerciseCount = item.exercitii?.length || 0;
            return (
              <View style={[styles.workoutCard, { backgroundColor: colors.surfaceBg, borderColor: colors.border }]}>
                <Pressable
                  ...
                  accessibilityRole="button"
                  accessibilityLabel={`${isOpen ? 'Închide' : 'Deschide'} ${item.nume}`}
                ...
                >
                  <View style={styles.workoutCopy}>
                    ...
                    <View style={styles.metaRow}>
                      ...
                      <Text style={[styles.meta, { color: colors.textSecondary }]}>{item.durata_min} min</Text>
                      <Text style={[styles.meta, { color: colors.textSecondary }]}>• {exerciseCount} exerciții</Text>
                      {item.is_local ? <Text style={[styles.localTag, { color: colors.warning }]}>local</Text> : null}
                    </View>
                  </View>
                  ...
                  <View style={[styles.exerciseList, { borderTopColor: colors.border }]}>
                    {(item.exercitii || []).map((exercise, exerciseIndex) => (
                      <View key={`${exercise.exercitiuId}-${exerciseIndex}`} style={styles.exercise}>
                        <Text style={[styles.exerciseName, { color: colors.textPrimary }]}>{exercise.name || exercise.nume}</Text>
...
const styles = StyleSheet.create({
  ...
  content: { width: '100%', maxWidth: 620, alignSelf: 'center', padding: 18 },
  topContent: { gap: 16 },
  summaryRow: { flexDirection: 'row', gap: 10 },
  summary: { flex: 1, minHeight: 76, borderRadius: 16, borderWidth: 1, alignItems: 'center', justifyContent: 'center' },
  ...
  empty: { marginTop: 12, padding: 28, borderRadius: 20, borderWidth: 1, alignItems: 'center' },
  emptyTitle: { fontSize: 16, fontWeight: '800', marginTop: 12 },
  emptyText: { fontSize: 13, lineHeight: 19, textAlign: 'center', marginTop: 5 },
  workoutCard: { borderRadius: 18, borderWidth: 1, marginTop: 12, overflow: 'hidden' },
  workoutHeader: { minHeight: 68, flexDirection: 'row', alignItems: 'center', padding: 15 },
  workoutCopy: { flex: 1 },
  workoutName: { fontSize: 15, fontWeight: '800' },
  metaRow: { flexDirection: 'row', alignItems: 'center', flexWrap: 'wrap', gap: 5, marginTop: 5 },
  meta: { fontSize: 12, fontWeight: '600' },
  localTag: { fontSize: 10, fontWeight: '900', textTransform: 'uppercase' },
  exerciseList: { borderTopWidth: 1, padding: 15 },
  exercise: { marginBottom: 14 },
  exerciseName: { fontSize: 14, fontWeight: '800', marginBottom: 7 },
  setList: { flexDirection: 'row', flexWrap: 'wrap', gap: 7 },
  setPill: { flexDirection: 'row', alignItems: 'center', gap: 5, borderRadius: 10, paddingHorizontal: 9, paddingVertical: 7 },
  setText: { fontSize: 11, fontWeight: '700' },
  setValue: { fontSize: 12, fontWeight: '800' },
  warmup: { fontSize: 9, fontWeight: '900' },
});
